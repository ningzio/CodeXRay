...
const ALGORITHMS: Record<string, AlgoConfig<any>> = {
  ...
  quick: {
    ...
    profile: {
      ...
      limitations: ['最坏情况 O(n²) (已排序数组，需随机化 Pivot 解决)', '不稳定 (改变相等元素的相对顺序)'],
    },
  ...
  bfs: {
    ...
    profile: {
      ...
      limitations: ['内存消耗较大 (需存储整层节点)', '无法处理带权图的最短路径'],
    },
    code: BFS_CODE,
  ...
  dfs: {
    ...
    profile: {
      ...
      limitations: ['不能保证找到最短路径', '深度过大可能导致栈溢出'],
    },
    code: DFS_CODE,
    getInitialData: () => SAMPLE_GRAPH,
    Visualizer: GraphVisualizer,
  ...
  dijkstra: {
    ...
    profile: {
      ...
      limitations: ['无法处理负权边 (需用 Bellman-Ford)', '不适用于存在负权环的图'],
    },
    code: DIJKSTRA_CODE,
